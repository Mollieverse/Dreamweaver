  ...
  return (
    <div className="flex items-center justify-center mb-6">
      <div
        ...
        style={{
          background: `radial-gradient(circle at 40% 40%, #fff9e6, #fde68a, #f59e0b)`,
...
function SleepTimer({ onStop }) {
  ...
  const start = (m) => {
    ...
    ref.current = setInterval(() => {
      setLeft((l) => {
        if (l <= 1) { clearInterval(ref.current); onStop(); return 0; }
        return l - 1;
      });
    }, 1000);
  };
    ...
    <div className="text-center">
      {left !== null && left > 0 ? (
        <div className="text-sm opacity-70">
          ...
          <button onClick={() => { clearInterval(ref.current); setLeft(null); setMins(null); }}
...
function ParentView({ profile, setProfile, stories, onGenerate, onSwitchChild, generating }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#fdf6f0] via-[#fef9f5] to-[#f0f4ff] font-sans">
      <style>{`
      ...
      <div className="px-5 pt-10 pb-6 bg-white/60 backdrop-blur-sm border-b border-stone-100 sticky top-0 z-10">
        <div className="flex items-center justify-between max-w-md mx-auto">
          <div>
            <h1 className="title-font text-2xl font-700 text-stone-800 leading-tight">
              ✨ DreamWeaver
            </h1>
      ...
      <div className="max-w-md mx-auto px-5 py-6 space-y-5">
        ...
        <div className="card bg-white rounded-3xl shadow-sm border border-stone-100 overflow-hidden">
          <div className="bg-gradient-to-r from-amber-50 to-rose-50 px-6 pt-5 pb-4">
            <div className="flex items-center justify-between mb-1">
              ...
              {!editing && <button onClick={() => setEditing(true)} className="text-xs text-amber-600 hover:text-amber-800 font-medium">Edit</button>}
            </div>
          </div>
          ...
          <div className="px-6 pb-6 pt-4">
            {editing ? (
              <div className="space-y-3">
                ...
                <button onClick={save}
                  className="w-full py-3 bg-stone-800 text-white rounded-2xl text-sm font-medium hover:bg-stone-700 transition-all mt-2">
                  Save Profile ✓
                </button>
              </div>
            ) : (
        ...
        {profile && !editing && (
          <div className="card" style={{animationDelay:"0.1s",opacity:0}}>
            <button onClick={onGenerate} disabled={generating}
              className="w-full py-5 rounded-3xl text-white font-medium text-lg shadow-lg transition-all active:scale-95 relative overflow-hidden"
        ...
        {/* Tech note */}
        <div className="card bg-indigo-50 rounded-2xl p-4" style={{animationDelay:"0.3s",opacity:0}}>
          <p className="text-xs text-indigo-600 font-medium mb-1">🛠 Implementation Guide</p>
          <p className="text-xs text-indigo-500 leading-relaxed">
            <strong>Stack:</strong> React + Tailwind (frontend) · Claude API (story gen) · Web Speech API / ElevenLabs (TTS) · Firebase/Supabase (auth + DB) · Vercel (deploy)<br/><br/>
            <strong>Data Flow:</strong> Child profile → Claude prompt → Story text → TTS → Audio stream → Playback<br/><br/>
            <strong>For production:</strong> Add Firebase Auth, Firestore for story history, ElevenLabs for premium voices
          </p>
        </div>
      </div>
    </div>
...
function ChildView({ story, profile, onBack }) {
  ...
  const readingTime = Math.ceil(totalWords / 120); // ~120 wpm slow narration
  ...
  const speak = useCallback(() => {
    ...
    const preferred = voices.find(v=>v.name.includes("Samantha")||v.name.includes("Karen")||v.name.includes("Moira")||v.lang==="en-GB") || voices[0];
    if (preferred) utter.voice = preferred;

    utter.onend = () => { setPlaying(false); setProgress(100); };
    utter.onboundary = (e) => {
      const pct = Math.min(100, (e.charIndex / story.text.length) * 100);
      setProgress(pct);
      const lineIdx = lines.findIndex((_, li) => {
        const charsSoFar = lines.slice(0,li+1).join(" ").length;
        return charsSoFar >= e.charIndex;
      });
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br ${t.bg} relative flex flex-col overflow-hidden`}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Crimson+Pro:ital,wght@0,300;0,400;1,300;1,400&family=DM+Sans:wght@300;400&display=swap');
        * { box-sizing:border-box; }
        .story-font { font-family: 'Crimson Pro', Georgia, serif; }
        @keyframes twinkle { 0%,100%{opacity:0.1} 50%{opacity:1} }
        @keyframes floatUp { 0%{opacity:0;transform:translateY(0) rotate(0deg)} 50%{opacity:0.7} 100%{opacity:0;transform:translateY(-100vh) rotate(20deg)} }
        @keyframes pulse-glow { 0%,100%{box-shadow:0 0 40px 20px ${t.glow}55,0 0 80px 40px ${t.glow}22} 50%{box-shadow:0 0 60px 30px ${t.glow}88,0 0 120px 60px ${t.glow}44} }
      ...
      <div className="relative z-10 flex items-center justify-between px-5 pt-8 pb-2">
        <button onClick={onBack} className="w-9 h-9 rounded-full bg-white/10 flex items-center justify-center text-white/70 hover:bg-white/20 transition-all backdrop-blur-sm">
          ←
        </button>
        <div className="flex gap-1.5">
          {Object.entries(THEMES).map(([k,v])=>(
            <button key={k} onClick={()=>setTheme(k)}
              className={`w-7 h-7 rounded-full text-sm transition-all ${theme===k?"ring-2 ring-white scale-110":""}`}
              title={v.label}>{v.emoji}</button>
          ))}
        </div>
        <button onClick={()=>setShowText(!showText)}
          className="w-9 h-9 rounded-full bg-white/10 flex items-center justify-center text-white/70 hover:bg-white/20 transition-all backdrop-blur-sm text-sm">
          📖
        </button>
      </div>
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-6 py-4">
        ...
        {/* Story title */}
        <h1 className="story-font text-white text-center text-2xl font-light italic mb-1 leading-tight px-4">
          {story?.title || "A Magical Bedtime Story"}
        </h1>
        ...
        {/* Play/Pause Button — BIG for child */}
        <button onClick={toggle}
          className="w-28 h-28 rounded-full flex items-center justify-center text-5xl shadow-2xl transition-all active:scale-90 relative"
          style={{
            background:`radial-gradient(circle at 35% 35%, ${t.accent}cc, ${t.glow}88)`,
            boxShadow:`0 0 50px ${t.glow}66, inset 0 2px 4px rgba(255,255,255,0.3)`,
          }}>
          <span style={{marginLeft: playing ? 0 : 6}}>{playing ? "⏸" : "▶"}</span>
        </button>

        <button onClick={restart}
          className="mt-4 text-white/40 hover:text-white/80 text-sm transition-all">
          ↺ Start over
        </button>
        ...
        {soundscape && <p className="text-white/30 text-xs mt-1">🎵 Background sounds in production use Web Audio API</p>}
      </div>
      ...
      {showText && (
        <div className="absolute inset-0 z-20 bg-black/80 backdrop-blur-sm flex flex-col" onClick={()=>setShowText(false)}>
          <div className="flex-1 overflow-y-auto px-6 pt-16 pb-24" onClick={e=>e.stopPropagation()}>
            ...
            {lines.map((line, i) => (
              <p key={i}
                className={`story-font text-lg leading-relaxed mb-3 transition-all duration-500 ${i===currentLine&&playing?"text-white line-highlight":"text-white/60"}`}>
                {line}
              </p>
            ))}
          </div>
          <div className="absolute bottom-6 left-0 right-0 flex justify-center">
            <button onClick={()=>setShowText(false)} className="px-6 py-2 bg-white/20 text-white rounded-full text-sm backdrop-blur-sm">
              ✕ Close text
            </button>
          </div>
        </div>
...
- End with the child character (${profile.name}) falling peacefully asleep
- Include vivid, dreamlike imagery
- Simple vocabulary suitable for age ${age}
Start with a creative title on the first line, then the story. Format: TITLE: [title]\n\n[story]`;
  ...
  const title = titleMatch ? titleMatch[1].trim() : `${profile.name}'s Dream Adventure`;
  const text = raw.replace(/TITLE:.+\n+/, "").trim();
  return { title, text, theme, date: new Date().toLocaleDateString("en-US",{month:"short",day:"numeric"}) };
}
...
export default function App() {
  ...
  const [stories, setStories] = useState(SAMPLE_HISTORY);
  ...
  if (view === "child" && story) {
    return <ChildView story={story} profile={profile} onBack={() => setView("parent")} />;
  }
  ...
  return (
    <>
      ...
      <ParentView
        ...
        onSwitchChild={handleSelectStory}
